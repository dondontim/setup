...
function setup_ssl() {
  ...
  # Only one is set
  certbot --nginx --non-interactive --agree-tos --hsts --staple-ocsp --redirect --cert-name "$CERT_NAME" --no-eff-email -m "$EMAIL"  -d "${PRIMARY_DOMAIN}" -d "mail.${PRIMARY_DOMAIN}" -d "www.${PRIMARY_DOMAIN}"  #-d "smtp.${PRIMARY_DOMAIN}" -d "imap.${PRIMARY_DOMAIN}" 
