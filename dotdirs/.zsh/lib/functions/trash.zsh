...
function bin_put binput binrm bin_mv trash_put trash-put() {
  # TODO(tim): mv is aliased to mv -i, so make a check here 
  # if file to move already exists with this name if yes, 
  # rename file we want to move to x copy -> x 2 -> x 3 etc and inform about it
  mv "$@" $TRASH
}